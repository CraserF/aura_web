...
- `npm run lint` is a required gate on the active artifact-pack rebuild branch; keep it in the standard verification gate before marking any phase release-ready.
- Artifact-pack rebuild status is split between implementation gates and product/release readiness. Phases 11 and 12 have foundation/create-routing proof only; document source-backed edits, spreadsheet runtime routing, deterministic workbook export, preview backfill, and in-app/manual workflow evidence remain open gates.
...
Active artifact-pack design rebuild: Phase 5, Phase 9, Phase 10, and Phase 13 implementation gates are complete on `codex/artifact-pack-design-rebuild`; Phases 11 and 12 are foundation/create-routing complete, not product-complete. The presentation path maps safe project `DESIGN.md`/rules or project color-theme roles into typed project-design metadata, shows a Project colours preview with ignored-line diagnostics, and applies those colours through the Editorial Stage pack adapter and compiler-owned CSS variables. The Editorial Stage visual gate is closed with CDP-captured 1280x720 screenshots across `editorial-magazine`, `modern-minimal`, and `data-utility`, plus render smoke assertions for fixed slide dimensions and background coverage, but fresh in-app generation and edit workflow validation remain release gates. Document-side proof and routing are in `document/executive-memo-v1`: executive-memo-like creates now compile through the pack compiler and persist source payload/manifest metadata, while source-backed edits and richer preview/export work remain open. Spreadsheet-side proof is in `spreadsheet/operating-model-v1`, which compiles structured workbook/table/formula/chart source through a spreadsheet-theme adapter, deterministic JSON compiler, formula materialization, declared column type/nullability validation, formula/cell/chart safety validation, and workbook validator; normal spreadsheet run plans do not claim pack-backed routing until spreadsheet runtime/export routing lands. Phase 13 now has a browseable Artifact Library from the toolbar using Aura-owned compiled examples and pack metadata, plus deterministic start-from-example for supported shipped examples, including shipped media and materialized spreadsheet rows; save-as-pack remains queued until pack-candidate authoring contracts are designed. See `docs/plan/aura_artifact_design_rebuild_implementation_checklist.md`.
