...
Active artifact-pack design rebuild: Phase 9 design-system mode is `Active/Next` on `codex/artifact-pack-design-rebuild`. The presentation path now maps safe project `DESIGN.md`/rules or project color-theme roles into typed project-design metadata, shows a Project colours preview with ignored-line diagnostics, and applies those colours through the Editorial Stage pack adapter and compiler-owned CSS variables. The first document-side adapter proof is in `document/executive-memo-v1`, which compiles a source-payload executive memo through a scoped document-token adapter, pack-owned CSS, and document-specific validation. Phase 10 presentation export/preview gates are complete: fixed-stage HTML/PDF/PPTX validation is in place, Editorial Stage exposes `examples/preview.png`, runtime-created presentation preview PNGs persist through project media plus `ProjectDocument.artifactPreview`, and `node scripts/run-presentation-preview-smoke.mjs` passed with a 1280x720 PNG at `media/artifacts/browser-preview-smoke-deck/artifact.preview.png`. Keep Phase 9 open until spreadsheet pack adapters prove the same shared-token contract, keep Phase 11 open until document runtime routing uses the pack compiler, and keep Phase 5 active until the logged screenshot fixes are implemented and rerun. See `docs/plan/aura_artifact_design_rebuild_implementation_checklist.md`.
