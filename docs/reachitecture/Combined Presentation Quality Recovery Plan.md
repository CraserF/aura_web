...
| 3A. CSS/design contract gate | `[x]` | Codex + Claude / 2026-04-28 | 7 CSS/design tests in `presentation-quality-checklist.test.ts`; full `npm test` — 109 files, 623 tests pass; `tsc --noEmit` clean | Appended-slide style reset corrected by Codex. Named CSS/design failure IDs now cover missing style systems, inline styles, external assets, reduced motion, viewport units, tiny type, animation budget, duplicate/reset style systems, weak continuity, and unscoped extensions. `buildCssDesignContractCheck` is wired into `buildPresentationQualityChecklist`; named failures flow once into `collectPresentationNamedFailures` → `evaluateAndRevise` deterministic revision. |
...
| 6. Queue real deck prompts by default | `[x]` | Claude + Codex / 2026-04-28 | `workflow-planner.test.ts` + `batchIntentDetection.test.ts` focused pass; `npm run typecheck`; full `npm test` — 109 files, 629 tests pass | Deck/presentation/keynote/slideshow/PowerPoint prompts now queue by default unless explicitly single-slide; rich deck prompts default to 5 slides, simple prompts to 3, local/Ollama defaults to 3 unless explicit structure/count is supplied; run plans attach narrative roles and continuity blueprints. |
| 7. Explicit runtime budgets | `[x]` | Claude + Codex / 2026-04-28 | `artifact-runtime.test.ts`, `presentation-runtime-policy.test.ts`, `presentation-runtime-workflow.test.ts`, and `quality-decision.test.ts` focused pass; `npm run typecheck`; full `npm test` — 109 files, 632 tests pass | Added `maxTotalRuntimeMs`, `maxRepairPasses`, `maxOptionalPolishPasses`, `maxToolLoopSteps`, and `budgetEnforcement: 'boundary-only'` to `ArtifactRuntimeMetricsBudget`; presentation repair, optional LLM polish, and ToolLoop correction boundaries now consult runtime budgets before starting optional work; frontier defaults to 120s/2 repairs/1 polish/5 tool steps, local/Ollama to 90s/1 repair/0 polish/3 tool steps. |
| 8. Local/Ollama calibrated parity | `[x]` | Claude + Codex / 2026-04-28 | `artifact-runtime.test.ts` — local plan asserts `safetyBlocksOutput: true`, `minimumScore: 78`, constrained budgets, and presentation signal IDs; `quality-decision.test.ts` — local safety blocks, local/frontier signal ID parity, and comparable presentation telemetry fields; `ollama-gemma4-baseline.md` documents presentation thresholds, telemetry expectations, and upgrade policy; full `npm test` — 109 files, 634 tests pass; `tsc --noEmit` clean | Local provider uses the same workflow, presentation quality signal IDs, telemetry shape, and safety gate (`safetyBlocksOutput: true`); local minimum score 78 vs frontier 86; 0 LLM polish passes vs 1; threshold upgrade requires benchmark evidence documented in `workflow-quality-benchmark.md`. |
| 9. Ollama benchmark harness | `[x]` | Claude + Codex / 2026-04-28 | `benchmark-harness.test.ts` covers composite scoring, consistency scoring, all 8 failure classifications, scorecard generation, required fixture coverage, and summary building without live Ollama; `npm run typecheck:benchmark`; `npm run typecheck`; full `npm test` — 110 files, 665 tests pass; empty-case `npm run benchmark:ollama` smoke proves the Vite SSR entrypoint, alias resolution, and scorecard output without live Ollama; real Ollama runs remain opt-in | `src/services/benchmarkHarness/` modules: scoring (55/25/10/10 weights), failure classifier (routing-bug / workflow-bug / provider-capability-mismatch / model-quality-limitation / prompt-tuning-issue / quality-depth / quality-visual / quality-continuity), scorecard generator, summary builder; `scripts/benchmark-ollama.ts` opt-in runner uses the normal presentation, document, and spreadsheet workflow paths with provider id `ollama`; env vars: `AURA_OLLAMA_MODEL`, `AURA_OLLAMA_BASE_URL`, `AURA_OLLAMA_CASES`, `AURA_OLLAMA_RERUNS`, `AURA_OLLAMA_JUDGE`; writes `summary.json`, `scorecard.md`, per-case `telemetry.json`, `events.json`, result JSON, and `output.html` or `output.json` to `logs/ollama-benchmark/<timestamp>/`. |
| 10. Outcome and manual Improve UX | `[x]` | Claude + Codex / 2026-04-28 | `qualityOutcomeLabel` in `renderRunResult.ts` maps all 4 quality decisions; `presentationHandler.ts` builds outcome-driven `presentationContent` and exposes `clarifyOptions` Improve action after `safe-budget-exhausted`; public progress labels stay in the simple Step 10 vocabulary without mislabeling document module work; 5 `qualityOutcomeLabel` tests and 3 public-progress-label tests in `workflow-progress.test.ts`; `ai-working-indicator.test.tsx` proves repair details stay out of the default progress UI; 2 run-result tests in `run-result.test.ts`; full `npm test` — 110 files, 675 tests pass; `tsc --noEmit` clean | Default chat content and progress labels are non-technical; advanced diagnostics stay in `structuredStatus.advancedDiagnostics`; Improve action is one explicit `clarifyOptions` entry, not a hidden second loop. |
| 11. Manual visual validation and release gate | `[~]` | Claude / 2026-04-30 | Step 11 release process is codified in `src/services/validation/releaseGates.ts`; benchmark runner compiles post-ExecutionMode cleanup (`npm run typecheck:benchmark`) and smoke-runs with empty case filter; focused benchmark run logged at `logs/ollama-benchmark/2026-04-30T15-47-06-278Z/`; release-smoke, scaffold guardrail, benchmark harness, and release-gate tests pass; `docs/validation/workflow-quality-benchmark.md` and `docs/validation/agent-in-app-qa-prototype.md` updated | Manual visual product gate still open: need one frontier single-slide, one frontier queued deck, one Ollama queued deck, and viewport captures (desktop wide/tablet/mobile) to close M5/M6 release evidence. |
