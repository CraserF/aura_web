...
5. **No external images.** Do NOT use Unsplash, Pexels, or any external image URLs. Use CSS gradients, inline SVGs, emoji (at 4-6em for visual impact), Bootstrap Icons, or geometric shapes. Only use \`<img>\` if the user explicitly provides a URL.
6. **No \`<img>\` tags with placeholder or stock URLs.** If you need a visual, create it with CSS, SVG, or emoji — NOT a CSS illustration of a real object (see gotchas).
