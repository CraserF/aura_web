...
export async function designEdit(
  ...
  const correctionAbortController = new AbortController();
  const correctionSignal = signal
    ? AbortSignal.any([signal, correctionAbortController.signal])
    : correctionAbortController.signal;
  const correctionTimeoutId = correctionPolicy.timeoutMs
    ? setTimeout(() => {
        correctionTimedOut = true;
        correctionAbortController.abort(new DOMException('Edit correction soft timeout reached', 'AbortError'));
      }, correctionPolicy.timeoutMs)
    : undefined;

  let agentResult;
  try {
    agentResult = await withRetry(() =>
      agent.generate({
        messages: validationMessages,
    ...
    if (
      correctionPolicy.mode === 'best-effort'
      && correctionTimedOut
      && countSlides(processedDraft) > 0
    ) {
      aiDebugLog('designer:edit', 'best-effort correction timeout reached, using streamed draft', {
        timeoutMs: correctionPolicy.timeoutMs,
        slideCount: countSlides(processedDraft),
        blockingCount: draftQa.blockingCount,
        continuityScore: continuity?.score,
      });
      onEvent({
        type: 'progress',
        message: 'Using the best available local edit draft to keep the workflow moving',
        pct: 84,
      });
      return {
        html: processedDraft,
