...
export function buildSeriesColors(primary: string, accent: string, count: number): string[] {
  const seed = [
    normalizeHexColor(primary, DEFAULTS.primary),
    normalizeHexColor(accent, DEFAULTS.accent),
    ...FALLBACK_SERIES,
  ];
  const colors: string[] = [];
...
export function buildChartTheme(input?: ChartThemeInput): ChartTheme {
  ...
  return {
    mode: resolved.mode,
    textColor: normalizeHexColor(resolved.text, DEFAULTS.text),
    mutedTextColor: normalizeHexColor(resolved.muted, DEFAULTS.muted),
    gridColor: resolved.mode === 'dark' ? 'rgba(148,163,184,0.28)' : 'rgba(100,116,139,0.18)',
    borderColor: resolved.border,
    backgroundColor: normalizeHexColor(resolved.background, DEFAULTS.background),
